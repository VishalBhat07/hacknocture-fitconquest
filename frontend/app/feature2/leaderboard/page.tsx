...
export default function Leaderboard() {
  ...
  const fetchChallenges = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/leaderboard/challenges`,
        {
  ...
  const fetchChallengeLeaderboard = async (id: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/leaderboard/challenge/${id}`,
        {
          headers: { "ngrok-skip-browser-warning": "true" },
        },
      );
  ...
  const fetchGlobalLeaderboard = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/leaderboard`,
        {
          headers: { "ngrok-skip-browser-warning": "true" },
        },
      );
  ...
  const getTeamColor = (name: string) =>
    name === "Red" ? "#ff8888" : "#88ccff";
  const getTeamBg = (name: string) =>
    name === "Red" ? "rgba(255,100,100,0.08)" : "rgba(100,200,255,0.08)";
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", textAlign: "center", marginBottom: "3rem" }}>
        <div className="page-icon" style={{ margin: "0 auto 1.5rem" }}>
          🏆
        </div>
        <h1>Leaderboards</h1>
        <p className="subtitle" style={{ margin: "0 auto" }}>
          Team rankings across all challenges
        </p>
      </div>
      ...
      >
        <button
          ...
          style={{
            ...
            fontSize: "0.9rem",
            background:
              view === "challenges" ? "var(--accent-1)" : "transparent",
              ...
              >
                ...
                >
                  <span style={{ fontSize: "2rem" }}>
                    {getExerciseIcon(c.exerciseType)}
                  </span>
                  <div>
                    <h3 style={{ fontSize: "1.3rem", margin: 0 }}>{c.title}</h3>
                    <span style={{ fontSize: "0.8rem", color: "#888" }}>
                      {c.exerciseType === "squat" && `${c.targetSquats} Squats`}
                      {c.exerciseType === "pushup" &&
                        `${c.targetPushups} Pushups`}
                      {c.exerciseType === "mixed" &&
                        `${c.targetSquats} Squats + ${c.targetPushups} Pushups`}
                    </span>
                  </div>
                </div>
                <div
                ...
                {c.teams.map((t: any) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "12px",
                      background: getTeamBg(t.teamName),
                      border: `1px solid ${getTeamColor(t.teamName)}22`,
                    }}
                  >
                    <div
                      style={{
                    ...
                    {t.completed && (
                      ...
                      >
                        ✅ Completed{" "}
                        {t.timeTakenFormatted
                          ? `in ${t.timeTakenFormatted}`
                          : ""}
                      </div>
                    )}
                  </div>
        ...
        <div style={{ width: "100%", maxWidth: "900px", margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
            <span style={{ fontSize: "2.5rem" }}>
              {getExerciseIcon(challengeLeaderboard.exerciseType)}
            </span>
            <h2 style={{ fontSize: "2rem", margin: "0.5rem 0" }}>
              {challengeLeaderboard.title}
            </h2>
            <div
              style={{
                display: "flex",
            ...
            >
              {(challengeLeaderboard.exerciseType === "squat" ||
                challengeLeaderboard.exerciseType === "mixed") && (
                <span
                  style={{
                    padding: "0.3rem 0.8rem",
                    borderRadius: "20px",
              ...
              {(challengeLeaderboard.exerciseType === "pushup" ||
                challengeLeaderboard.exerciseType === "mixed") && (
                <span
                  style={{
                    padding: "0.3rem 0.8rem",
            ...
            {challengeLeaderboard.teams.map((team: any, idx: number) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: "20px",
                  background: "var(--card-bg)",
                  border: `2px solid ${idx === 0 && challengeLeaderboard.winnerTeam ? "rgba(250,204,21,0.4)" : "var(--card-border)"}`,
                  position: "relative",
                  overflow: "hidden",
                }}
              >
                  ...
                  >
                    <div
                      style={{
                        fontSize: "2rem",
                        fontWeight: "bold",
                        color: team.completed ? "#4ade80" : "#f59e0b",
                      }}
                    >
                      {team.completed ? "✅" : "⏳"}
                    </div>
      ...
      {view === "global" && (
        ...
        >
          {users.length === 0 ? (
            <p style={{ padding: "3rem", textAlign: "center", color: "#888" }}>
